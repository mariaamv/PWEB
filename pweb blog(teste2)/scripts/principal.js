...
function carregarArtigos(posts) {
    ...
    posts.forEach(post => {
        ...
        postElement.dataset.id = post.id; // id como "8dac"
...
// ⭐ EVENTO DE CLIQUE ÚNICO (SEM DUPLICAR)
document.addEventListener("click", e => {
    const alvo = e.target.closest(".abrir-post"); // pega o post clicado
    if (!alvo) return;

    const id = alvo.dataset.id; // pega o data-id
    if (!id) return;

    // verifica se existe uma página específica no objeto fotos
    const pagina = fotos[id];
    if (pagina) {
        // abre a página específica
        window.location.href = "pages/" + pagina;
    } else {
        // fallback: abre detalhes.html com o id
